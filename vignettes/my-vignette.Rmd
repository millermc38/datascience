...
If you've learned ANOVA and regression in a field outside of statistics, you might not be fully aware that ANOVA is actually just a special case of regression wherein the intercept is dropped and all variables are considered as factors. We can use the same call as in the OLS section above, but amend the intercept argument (by turning it off). Note that the statistical tests are not all that meaningful here (edits currently underway). In fact, what really distinguished ANOVA from regression is not the model fit process, but generally the analysis that follow. For example, we typically want to look at contrasts. Which groups are different? I endeavor to include these contrast in future updates.
